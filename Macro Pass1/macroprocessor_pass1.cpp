    ...
    void passOne()
    {
        ...
        if (file.is_open())
        {
            while (!file.eof())
            {
                ...
                /*If inside a macro and it's the first line, extract the macro name and initialize counters.
Process parameters and defaults:
Identify positional parameters (those starting with '&').
Identify keyword parameters (those followed by "=").
Update counters (pcounter, kcounter).
Populate the Parameter Name Table (PNTAB) and Keyword Parameter Default Table (KPDTAB).*/
                else if (firstLine && insideMacro)
                {
                    ...
                    for (int i = 1; i < words.size(); i++)
                    {
                        ...
                        else if (words[i] == "=")
                        {
                            ...
                            else if (i == words.size() - 1 || words[i + 1].at(0) == '&') // positional + keyword
                            {
                ...
                /*If inside a macro but not the first line, process the actual macro definition:
Replace parameters with their positions in the Parameter Name Table.
Add the processed line to the Macro Definition Table (MDT).
Update counters (mcounter).*/
                else if (!firstLine && insideMacro)
                {
